...
void RedrawMenuCursor(u8 oldPos, u8 newPos)
{
    ...
    FillWindowPixelRect(sMenu.windowId, PIXEL_FILL(1), sMenu.left + 30, sMenu.optionHeight * oldPos + sMenu.top, width, height);
    AddTextPrinterParameterized(sMenu.windowId, sMenu.fontId, gText_SelectorArrow3, sMenu.left + 30, sMenu.optionHeight * newPos + sMenu.top, 0, 0);
}
...
void PrintMenuTable(u8 windowId, u8 itemCount, const struct MenuAction *menuActions)
{
    ...
    for (i = 0; i < itemCount; i++)
        AddTextPrinterParameterized(windowId, 1, menuActions[i].text, 24, (i * 16) + 1, TEXT_SKIP_DRAW, NULL);
